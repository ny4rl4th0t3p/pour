...
func (m *Manager) GetActive(chainID string) (ChainSnapshot, bool) {
	...
	return ChainSnapshot{Info: c.Info(), Drip: c.Drip(), IBCTimeout: c.IBCTimeout(), IBCDrips: c.IBCDrips()}, true
}
...
func (m *Manager) ListActive() []ChainSnapshot {
	...
	for _, c := range m.chains {
		out = append(out, ChainSnapshot{Info: c.Info(), Drip: c.Drip(), IBCTimeout: c.IBCTimeout(), IBCDrips: c.IBCDrips()})
	}
	m.mu.RUnlock()
